...
export default function App() {
  ...
  const chapters = [
    { num: 1, title: '철학적 토대와 시스템 미학의 융합', sub: 'PDF-DS 서론' },
    { num: 2, title: '청사진 그리드와 멀티 스케일 여백 시스템', sub: 'Blueprint Grid & Spacing' },
    { num: 3, title: '정밀 타이포그래피 및 레이아웃 정렬 명세', 초정밀: 'Typography & Layout' },
    { num: 4, title: '아크로매틱 컬러 체계와 펑셔널 레드', sub: 'Achromatic Color & Functional Red' },
    { num: 5, title: '초정밀 버튼 설계 및 형태 모핑 상호작용', sub: 'Morphing Button' },
    { num: 6, title: '비대칭 스플릿 스크린 레이아웃', sub: 'Asymmetric Split Screen' },
    { num: 7, title: '통합 소프트웨어 아키텍처 및 토큰 구현', sub: 'Architecture & Token' },
    { num: 8, title: '최종 통합 실무 체크리스트 및 검수 수칙', sub: 'QA & Checklist' },
  ];
      ...
      >
        <div className="pdf-content-relative pdf-p-300">
          <div className="pdf-mb-300">
            ...
            <h1 className="pdf-text-heading-32 pdf-mb-100">
              PDF-DS System
            </h1>
      ...
      >
        <div className="pdf-panel pdf-grid-bg">
          <div className="pdf-content-relative pdf-flex-row pdf-justify-between pdf-items-center">
            ...
            <div className="pdf-text-label-14-mono pdf-text-muted">
              {Math.round((activeChapter / 8) * 100)}% PROGRESS
            </div>
          </div>
        </div>
        ...
        <div className="pdf-footer">
          ...
          <span className="pdf-text-label-14-mono pdf-text-muted">
            STAGE 0{activeChapter} / 08
          </span>
          <button
            disabled={activeChapter === 8}
            onClick={() => handleChapterSwitch(activeChapter + 1)}
            className="pdf-secondary-btn"
            style={{ borderColor: activeChapter !== 8 ? 'var(--color-functional-red)' : undefined, color: activeChapter !== 8 ? 'var(--color-functional-red)' : undefined }}
          >
            NEXT →
          </button>
        </div>
