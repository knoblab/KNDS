  ...
  return (
    <div className="bg-zinc-50 border border-zinc-200 p-4 shrink-0 rounded-lg my-8 flex flex-col justify-center overflow-hidden relative shadow-sm">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex bg-zinc-200 p-1 rounded-md shrink-0">
          <button
            onClick={() => setViewMode('legacy')}
            className={`px-3 py-1.5 text-xs font-bold font-mono rounded transition-colors ${
              viewMode === 'legacy' ? 'bg-white text-zinc-900 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'
            }`}
          >
            LEGACY WEB
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-bold font-mono rounded transition-colors ${
              viewMode === 'pdfds' ? 'bg-red-750 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-700'
            }`}
          >
            PDF-DS
          </button>
      ...
      <div className="text-xs text-zinc-500 mb-6 leading-relaxed">
        {viewMode === 'legacy' ? (
        ...
        <div className="flex-1 bg-white relative overflow-hidden flex transition-all duration-500">
          ...
          {viewMode === 'legacy' ? (
            ...
            <div className="w-full flex-1 flex flex-col items-center relative animate-fade-in">
              {/* Header */}
              <div className="w-full h-14 border-b border-zinc-200 bg-white shrink-0 flex items-center px-6 justify-between shadow-sm z-20">
              ...
              {/* Layout Wrapper */}
              <div className="flex w-full flex-1 relative bg-zinc-100 z-10">
                {/* Left Letterbox */}
                <div className="flex-1 bg-red-50/40 border-r-2 border-red-200 border-dashed flex flex-col justify-center items-center text-center">
                  <span className="text-red-500 text-[10px] font-bold">낭비되는</span>
                  <span className="text-red-500 text-[10px] font-bold">여백</span>
                </div>
                
                {/* Main Content (Narrow and Stretched) */}
                <div className="w-[50%] bg-white shadow-lg flex flex-col items-center pt-8 px-6 pb-2 overflow-hidden relative">
                  <span className="text-xs font-bold text-red-600 border border-red-200 leading-none bg-red-50 px-2 py-1 rounded absolute top-2 right-2 animate-pulse">
                    수직 압축됨
                  </span>
                  <div className="w-3/4 h-6 bg-zinc-200 rounded mb-4"></div>
                  <div className="w-full h-3 bg-zinc-100 rounded mb-2"></div>
                  <div className="w-[90%] h-3 bg-zinc-100 rounded mb-2"></div>
                  <div className="w-[95%] h-3 bg-zinc-100 rounded mb-6"></div>
                  <div className="w-full h-32 bg-zinc-50 border border-zinc-200 rounded shrink-0"></div>
                  ...
                  <div className="w-full h-16 bg-gradient-to-b from-white/0 via-white/80 to-white absolute bottom-0 left-0 flex justify-center items-end pb-2 point-events-none">
                     <span className="text-[9px] bg-zinc-800 text-white px-2 py-1 rounded-full flex gap-1">▼ 무한 스크롤</span>
                  </div>
                </div>

                {/* Right Letterbox */}
                <div className="flex-1 bg-red-50/40 border-l-2 border-red-200 border-dashed flex flex-col justify-center items-center text-center">
                  <span className="text-red-500 text-[10px] font-bold">낭비되는</span>
                  <span className="text-red-500 text-[10px] font-bold">여백</span>
                </div>
              </div>
               ...
               {/* Footer */}
               <div className="w-full h-12 bg-zinc-800 shrink-0 z-20 flex justify-center items-center">
            ...
            <div className="w-full flex-1 flex relative animate-fade-in">
              {/* PDF-DS LEFT 38% */}
              <div className="w-[38%] bg-zinc-100 border-r border-zinc-300 h-full flex flex-col">
                {/* PDF-DS Header Area */}
                ...
                <div className="p-4 flex flex-col gap-3 flex-1 overflow-y-auto">
                   ...
                   <div className="w-full h-24 bg-white border border-zinc-200 rounded-sm mt-2 flex flex-col gap-2 p-3">
                     <div className="w-3/4 h-2 bg-zinc-100"></div>
                     <div className="w-1/2 h-2 bg-zinc-100"></div>
                   </div>
                      <div className="mt-auto pt-4">
                     <div className="w-full h-10 rounded-full bg-red-750 flex items-center justify-center">
                        <div className="w-4 h-4 bg-white/20 rounded-full mr-2"></div>
                        <span className="w-12 h-2 bg-white/40 rounded"></span>
                     </div>
                   </div>
                </div>
              </div>
              ...
              {/* PDF-DS RIGHT 62% */}
              <div className="w-[62%] h-full bg-white flex flex-col border-l border-zinc-200/50">
                 {/* 62 Canvas Header */}
                 ...
                 <div className="flex-1 p-8 grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-zinc-50 border border-zinc-100 h-32 rounded-lg p-4">
