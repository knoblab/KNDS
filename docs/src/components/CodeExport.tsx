...
export default function CodeExport() {
  ...
  const codeBlocks = {
    web: `/* KNDS Core Web-Token Stylesheet */
:root {
  --space-025: 2px;
  --space-050: 4px;
  --space-100: 8px;
    ...
    <div className="knds-panel">
      <div className="knds-panel-header knds-flex-row knds-justify-between knds-items-center">
        <div>
          ...
          <h4 className="knds-text-label-16" style={{ fontWeight: 'bold' }}>
            크로스플랫폼 통합 토큰 및 코드 명세 보드 (JSON & Code Exporter)
          </h4>
        </div>
        <Terminal className="knds-text-muted" style={{ width: 16, height: 16 }} />
      </div>
      ...
      <div className="knds-flex-row knds-items-center knds-justify-between knds-gap-150 knds-mb-200" style={{ flexWrap: 'wrap' }}>
        <div className="knds-flex-row knds-bg-secondary knds-p-050 knds-border" style={{ borderRadius: 8 }}>
          <button
        ...
        <button
          onClick={handleCopy}
          className="knds-text-label-14-mono knds-flex-row knds-items-center knds-gap-050"
          style={{
      ...
      <div className="knds-p-200 knds-border" style={{ backgroundColor: '#09090b', color: 'var(--color-text-secondary)', borderRadius: 8, fontFamily: 'var(--font-mono)', fontSize: 11, lineHeight: 1.6, position: 'relative', overflowX: 'auto', maxHeight: 340 }}>
        <div className="knds-border" style={{ position: 'sticky', top: 0, right: 0, textAlign: 'right', backgroundColor: 'rgba(9, 9, 11, 0.8)', padding: '4px 8px', borderRadius: 4, display: 'inline-block', float: 'right', fontSize: 9, color: 'var(--color-text-secondary)', textTransform: 'uppercase' }}>
          {platform === 'web' ? 'CSS / Variables' : platform === 'android' ? 'KOTLIN / COMPOSE' : 'SWIFT / SWIFTUI'}
        </div>
        <pre className="knds-selectable" style={{ whiteSpace: 'pre', margin: 0, paddingBottom: 16 }}>{codeBlocks[platform]}</pre>
      </div>

      <div className="knds-flex-row knds-items-center knds-gap-100 knds-mt-200" style={{ backgroundColor: '#ecfdf5', borderColor: '#a7f3d0', borderStyle: 'solid', borderWidth: 1, padding: 12, borderRadius: 8, color: '#065f46' }}>
        <ShieldCheck style={{ width: 20, height: 20, color: '#047857', flexShrink: 0 }} />
        <span className="knds-text-copy-14" style={{ fontSize: 11, fontWeight: 500 }}>
