...
export default function PageSidebar({ onPreview }: { onPreview: () => void }) {
  ...
  return (
    <aside className="knds-sidebar" style={{ width: '25%', borderRight: '1px solid var(--color-border-default)', display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="knds-content-relative knds-p-300" style={{ flex: 1, overflowY: 'auto' }}>
            ...
            {sortedCategories.map(cat => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="knds-flex-row knds-items-center knds-gap-100" style={{ flex: 1 }} onClick={(e) => editingCat === cat && e.stopPropagation()}>
                      {editingCat === cat ? (
                        <div className="knds-flex-row knds-items-center knds-gap-050" style={{ width: '100%' }}>
                          <input
                            autoFocus
                            type="text"
                            value={editCatVal}
                          ...
                          />
                          <button 
                            className="knds-text-label-14-mono knds-text-red knds-font-bold" 
                    ...
                    <div className="knds-flex-row knds-items-center knds-gap-100" onClick={(e) => e.stopPropagation()}>
                      {editingCat !== cat && cat !== '' && (
                        <>
                          <button 
                            className="knds-text-label-14-mono knds-text-muted"
                          ...
                          <button 
                            className="knds-text-label-14-mono knds-text-red"
                  ...
                  {/* Pages list */}
                  {!isCollapsed && (
                    <div className="knds-flex-col knds-gap-050" style={{ paddingLeft: '8px', borderLeft: '1px solid var(--color-border-default)', marginLeft: '8px', minHeight: catPages.length === 0 ? '24px' : 'auto' }}>
                      {catPages.length === 0 ? (
                        <div className="knds-text-copy-13-mono knds-text-muted" style={{ padding: '8px', fontSize: '12px' }}>페이지를 여기로 드래그하세요</div>
                      ) : (
                        catPages.map(page => {
                            ...
                            >
                              <div className="knds-flex-row knds-items-center knds-gap-150 knds-overflow-hidden knds-w-full" style={{ padding: '2px 0' }}>
                                ...
                                <div className="knds-flex-col knds-overflow-hidden" style={{ flex: 1 }}>
                                  {editingId === page.id ? (
                                    <div className="knds-flex-col knds-gap-050 knds-w-full" onClick={(e) => e.stopPropagation()}>
                                      <input
                                        autoFocus
                                        type="text"
                                        value={editTitle}
                                      ...
                                      <input
                                        ...
                                        onKeyDown={(e) => e.key === 'Enter' && saveEdit(page.id)}
                                      />
                                      <div className="knds-flex-row knds-gap-100 knds-mt-100" style={{ marginTop: '8px' }}>
                                        <button 
                                          className="knds-btn-primary knds-btn-xs"
                                ...
                                {/* Action buttons */}
                                {editingId !== page.id && (
                                  <div className="knds-flex-row knds-gap-100" onClick={(e) => e.stopPropagation()}>
                                    <button 
                                      className="knds-text-label-14-mono knds-text-muted" 
      ...
      <div className="knds-p-200 knds-border-top knds-flex-col knds-gap-100" style={{ backgroundColor: 'var(--color-bg-secondary)' }}>
        {showAddCat ? (
          <div className="knds-flex-col knds-gap-050 knds-mb-100" style={{ width: '100%' }}>
            <input
              autoFocus
              type="text"
              className="knds-input knds-input-sm"
            ...
            />
            <div className="knds-flex-row knds-gap-100" style={{ marginTop: '4px' }}>
              <button className="knds-btn-primary knds-btn-xs" style={{ borderRadius: '4px', height: '28px' }} onClick={handleCreateCategory}>추가</button>
              <button className="knds-secondary-btn knds-btn-xs" style={{ borderRadius: '4px', height: '28px', padding: '0 8px' }} onClick={() => setShowAddCat(false)}>취소</button>
            </div>
          </div>
        ) : (
          <button className="knds-secondary-btn knds-w-full" onClick={() => setShowAddCat(true)}>
            + 새 대분류 추가
          </button>
        )}
        
        <button className="knds-btn-primary knds-w-full" onClick={handleAddPage}>
          + 새 페이지 추가
        </button>
        <button className="knds-secondary-btn knds-w-full" onClick={onPreview}>
          미리보기 모드
        </button>
        <button className="knds-secondary-btn knds-w-full" onClick={handleExportZip}>
          ZIP 파일로 내보내기
        </button>
        <button className="knds-secondary-btn knds-w-full" onClick={() => fileInputRef.current?.click()}>
          ZIP 파일 불러오기
        </button>
        <input 
          type="file" 
